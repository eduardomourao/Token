...
- PostgreSQL CI exposed a reserved-word defect in the original OpenCode Go migration: the `window` column was quoted in the table definition but not in its check constraint. The migration and model now use SQL `"window"`; the regression test protects the source representation. Docker Desktop is unavailable locally, so PostgreSQL execution remains a CI-only verification for this workstation.
- Legacy Alembic revision remapping can encounter a database whose schema is already at head while its version marker was replaced by an old alias. The upgrade flow now verifies zero schema drift and stamps head instead of replaying historical DDL. The two direct remapping regressions pass.
- Linux CI also showed two repository shell scripts had mode `100644`, which prevents Helm smoke and wrapper tests from executing. Their Git index modes are restored to executable.
