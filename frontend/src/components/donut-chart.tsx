...
export type DonutChartProps = {
  ...
  /** Compact, legend-free ring for space-constrained kiosk panels. */
  compact?: boolean;
};
...
export function DonutChart({ items, total, centerValue, title, subtitle, safeLine, centerLayout = "remaining", compact = false }: DonutChartProps) {
  ...
  if (compact) {
    ...
    const compactMargin = 3;
    const compactCenter = 57;
    const compactInnerRadius = 40;
    const compactOuterRadius = 54;
    return (
      <div className="relative h-[120px] w-[120px] overflow-visible" data-testid="compact-donut-chart">
        <PieChart width={compactSize} height={compactSize} margin={{ top: compactMargin, right: compactMargin, bottom: compactMargin, left: compactMargin }}>
          <Pie
            data={chartData}
            cx={compactCenter}
            cy={compactCenter}
            innerRadius={compactInnerRadius}
            outerRadius={compactOuterRadius}
            startAngle={90}
            endAngle={-270}
            dataKey="value"
            stroke="none"
          ...
          >
            {chartData.map((entry) => <Cell key={entry.id} fill={entry.fill} />)}
          </Pie>
        </PieChart>
        {safeLine && safeLine.riskLevel !== "safe" ? (
          <svg aria-hidden="true" className="pointer-events-none absolute inset-0" width={compactSize} height={compactSize} viewBox={`0 0 ${compactSize} ${compactSize}`}>
            <SafeLineTick cx={compactCenter + compactMargin} cy={compactCenter + compactMargin} safePercent={safeLine.safePercent} riskLevel={safeLine.riskLevel} innerRadius={compactInnerRadius} outerRadius={compactOuterRadius} isDark={isDark} />
          </svg>
        ) : null}
      </div>
    );
