...
SESSION_AFFINITY_MIGRATION = (
    REPOSITORY_ROOT / "supabase" / "migrations" / "20260902003000_hosted_proxy_session_affinity.sql"
)
...
WEBSOCKET_SPOOL_FIX_MIGRATION = next(
    REPOSITORY_ROOT.glob("supabase/migrations/*_fix_hosted_proxy_websocket_spool_cleanup.sql")
)
WEBSOCKET_SPOOL_SCHEDULE_MIGRATION = next(
    REPOSITORY_ROOT.glob("supabase/migrations/*_schedule_hosted_proxy_websocket_spool_purge.sql")
)
WEBSOCKET_SPOOL_STATUS_MIGRATION = next(
    REPOSITORY_ROOT.glob("supabase/migrations/*_add_hosted_websocket_spool_status.sql")
)
...
def test_private_proxy_import_bridges_are_service_role_only() -> None:
    ...
    assert (
        "revoke all on function public.hosted_proxy_upsert_credentials(jsonb) from public, anon, authenticated" in sql
    )
...
def test_hosted_proxy_usage_refresh_keeps_credentials_private_and_is_scheduled() -> None:
    ...
    assert (
        "revoke all on function public.hosted_proxy_accounts_for_usage_refresh(uuid) "
...
def test_hosted_proxy_rate_limit_transition_is_service_role_only() -> None:
    ...
    assert (
        "revoke all on function public.hosted_proxy_mark_rate_limited(uuid, text, bigint) "
        "from public, anon, authenticated" in sql
    )
...
def test_hosted_proxy_session_affinity_is_private_and_hash_only() -> None:
    ...
    assert (
        "revoke all on function public.hosted_proxy_session_account(uuid, text), "
        "public.hosted_proxy_bind_session(uuid, text, text) from public, anon, authenticated" in sql
    )
...
def test_hosted_websocket_spool_is_private_owner_scoped_and_bounded() -> None:
    ...
    assert "force row level security" in sql
    assert (
        "revoke all on table app.hosted_proxy_websocket_spools, "
        "app.hosted_proxy_websocket_events from anon, authenticated" in sql
    )
