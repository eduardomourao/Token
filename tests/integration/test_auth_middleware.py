...
async def _assert_guest_write_denied(client: AsyncClient) -> None:
    read_opencode_monitor = await client.get("/api/opencode-go-usage/")
    assert read_opencode_monitor.status_code == 200

    blocked_opencode_configuration = await client.put(
        "/api/opencode-go-usage/configuration",
        json={"apiKey": "guest-key"},
    )
    assert blocked_opencode_configuration.status_code == 403
    assert blocked_opencode_configuration.json()["error"]["code"] == "read_only_access"

    blocked_opencode_refresh = await client.post("/api/opencode-go-usage/refresh")
    assert blocked_opencode_refresh.status_code == 403
    assert blocked_opencode_refresh.json()["error"]["code"] == "read_only_access"

    blocked_opencode_removal = await client.delete("/api/opencode-go-usage/configuration")
    assert blocked_opencode_removal.status_code == 403
    assert blocked_opencode_removal.json()["error"]["code"] == "read_only_access"
