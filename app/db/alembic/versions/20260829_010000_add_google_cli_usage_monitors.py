...
def upgrade() -> None:
    ...
    if not sa.inspect(op.get_bind()).has_table("gemini_usage_sample"):
        op.create_table(
            "gemini_usage_sample",
    ...
    _create_index_if_missing("ix_antigravity_usage_sample_captured_at", "antigravity_usage_sample", ["captured_at"])
    _create_index_if_missing(
        "ix_antigravity_usage_sample_group_captured", "antigravity_usage_sample", ["group", "captured_at"]
    )
