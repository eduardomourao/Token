...
async def run_retention_pass(*, now: datetime | None = None) -> dict[str, int]:
    ...
    monitor_cutoff = now - timedelta(days=90)
    await _prune_opencode_go_usage_samples(monitor_cutoff)
    await _prune_google_cli_usage_samples(monitor_cutoff)
...
async def _prune_opencode_go_usage_samples(cutoff: datetime) -> int:
