...
| 2026-09-02 | Native `/v1/responses` initially bypassed the WebSocket adapter | 1 | Vercel selects the physical `api/v1/responses.ts` function before its rewrite. Keep the HTTP relay in that physical handler and branch to the gateway only for `GET` plus `Upgrade: websocket`; aliases re-export it. |
| 2026-09-02 | CI lint/type gates failed across repository style, SQLAlchemy result typing, and POSIX-only code | 1 | Applied behavior-preserving style fixes, structural fetcher protocols, safe row-count access, portable smoke cleanup, and Windows-only skips for fork tests. Ruff and Ty now pass; focused regression suite: 43 passed, 2 Windows skips. |
| 2026-09-02 | New CI run failed Helm smoke and PostgreSQL migration jobs while other jobs were still running | 1 | Captured exact failing job ids and step names; defer log extraction until GitHub completes the run because the platform does not expose failed logs while the overall run is active. |
| 2026-09-02 | PostgreSQL rejected unquoted `window` in OpenCode Go migration check constraint | 1 | Quote the reserved identifier in migration and ORM metadata; add regression coverage. Docker Desktop is not running locally, so the complete PostgreSQL migration proof stays in GitHub Actions. |
| 2026-09-02 | Linux CI could not execute Helm smoke or Codex wrapper scripts | 1 | Restore executable Git index modes for `scripts/helm-kind-smoke.sh` and `.agents/skills/codex-review-loop/scripts/codex-subagent.sh`. |
| 2026-09-02 | Legacy revision alias on a physically head schema replayed historical DDL | 1 | After legacy remap, if schema drift is empty, stamp Alembic head instead of replaying migrations; both direct remap regressions pass. |
