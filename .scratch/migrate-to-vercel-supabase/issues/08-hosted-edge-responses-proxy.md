...
o contrato HTTP de Responses, e mantém WebSocket, replay,
...
atualização de quota a partir do upstream, o failover no mesmo request e o
replay continuam fora deste ticket e não são inferidos a partir de dados
históricos antigos.
...
segue ao upstream ChatGPT. O `x-codex-session-id`, quando fornecido e limitado
a 512 caracteres, também termina na Edge Function para seleção por afinidade e
é explicitamente removido antes do upstream.
